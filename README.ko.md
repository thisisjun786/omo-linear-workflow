...
설명합니다. [docs/policy](docs/policy/pull-requests.md)의 정책은 브랜치 규칙을
정합니다. PR은 `dev`를 대상으로 하고 merge commit으로 병합하며, `main`은 릴리즈된
소스이고 소유자가 승인한 릴리즈만 `main`을 전진시킵니다. GitHub에는 버그·제안
양식과 PR 템플릿이 제공됩니다.
